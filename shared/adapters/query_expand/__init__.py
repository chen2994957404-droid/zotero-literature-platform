# -*- coding: utf-8 -*-
"""query_expand · 检索式扩展基础件（原子能力：一个研究问题 → 多个互补的英文检索式）
...
## 为什么住在 adapters 而不是 domain 或某个工具里（R3 窗，2026-08-30）

- **不能住在某个工具里**：`tools/askworld` 用 `to_english`、`tools/discover` 用 `expand`，
  两个工具都要 —— 按下沉规则（≥2 个工具用到才允许下沉）它必须下沉。
- **不能住 domain**：它 import `llm_client`，而 domain 环不许 import adapters
  （否则没法离线测试）。
- 于是落在 adapters，与 `snowball`（组合 openalex 的更高层外部能力）同类。
