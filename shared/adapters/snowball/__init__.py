# -*- coding: utf-8 -*-
"""snowball · 引用雪球基础件（原子能力：种子文献 → 沿引用网络扩展出的相关文献）
...
# HTTP、重试、摘要还原、字段归一，统一由 shared.adapters.openalex 负责 ——
...
def _get(url, timeout=45, retries=2):
    """GET + 退避重试。实现在 shared.adapters.openalex，这里只把异常翻译成 SnowballError。"""
    try:
...
def _abstract(inv):
    """OpenAlex 摘要是倒排索引，还原成正常文本（实现见 shared.adapters.openalex）。"""
    return openalex.restore_abstract(inv)


def _norm(w):
    """OpenAlex work → 本平台统一的文献字典（归一在 shared.adapters.openalex，这里只补 unique_id）。"""
