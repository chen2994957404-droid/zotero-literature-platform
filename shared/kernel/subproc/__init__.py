# -*- coding: utf-8 -*-
"""subproc · 子进程调用基础件（原子能力：跑一条外部命令，安静、带超时、编码正确）
...
**为什么做成模块而不是逐处修**：
逐处修只解决今天这 17 处；做成模块后，「调子进程」这件事只有一个正确入口，
以后任何新代码复用它就自动不弹窗。从源头杜绝优于事后补救。
（体检里另有一项会揪出绕过本模块的裸调用，防止规范腐坏。）

原子模块的特征：只做「执行一条命令并拿回结果」这一件不可再分的事。

用法：
    from shared.kernel.subproc import run, spawn
