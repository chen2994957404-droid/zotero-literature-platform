# -*- coding: utf-8 -*-
"""cli · 命令行参数解析基础件（原子能力：全项目只有一种取参数的方式）
...
设计原则：
  - 只读 sys.argv，不用 argparse —— 本项目参数都很简单（几个位置参数 + 几个 --开关），
    argparse 的 help/子命令等重装备用不上，反而引入一套新语法（符合架构准则判据：能自己做的稳定部分自己做）
  - 取不到参数返回默认值，**绝不抛异常** —— 缺参时由脚本业务逻辑自己判断（很多脚本允许无参运行）

用法：
    from shared.kernel.cli import pos, flag, opt, opts, positionals
