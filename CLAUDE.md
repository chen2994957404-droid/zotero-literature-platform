...
**先读 `docs/explain/架构宪法_第一性原理.md`** —— 它定义整个系统怎么构筑：公理→定理→组合，
三条铁律，以及「按稳定性决定自己做还是用现成」的首要判据。任何改动服从它。

## 接手先读（别边探边拼）

`docs/变更记录.md`（最新状态）→ `docs/incidents/踩坑记录.md`（已知坑）→
`docs/explain/架构宪法_第一性原理.md` → `docs/reference/数据契约.md`。别走到哪读到哪、靠猜（教训见踩坑 #14）。

## ⚠ 正在进行架构重构 v2（2026-08-26 起）

v2（四环）已被本次「按工具切片」的重构取代 —— **现在的唯一施工文件是 `REBUILD.md`**，
台账在它第五节。`docs/explain/架构重构_v2总体设计.md` 只作历史参考，别照它施工。
...
`docs/reference/架构总览.md`（数据流）· `docs/reference/演进规划与模块地图.md` ·
`docs/explain/对标报告_我们的思路 vs 前沿.md` · `docs/explain/积木采购清单_可借鉴的开源积木.md` ·
`docs/explain/正文数据关联_方向说明.md` · `docs/reference/视觉模型选择_参考.md` · `docs/待办与需求.md`
