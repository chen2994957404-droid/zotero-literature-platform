...
用法（--band 必填，因为一条窄带一个库）:
    python 找新文献/方向地图.py bands                        # 现有窄带
    python 找新文献/方向地图.py seeds   --band impact         # 三路取种子（联网）
    python 找新文献/方向地图.py wechat  --band X --dir <目录>  # 从公众号 md 取种子
    python 找新文献/方向地图.py build   --band impact         # 建图（联网，十几分钟）
    python 找新文献/方向地图.py cluster --band impact         # 聚类（纯本地，可反复调）
    python 找新文献/方向地图.py report  --band impact --out 地图.txt
    python 找新文献/方向地图.py stats   --band impact

    --min-df N     骨干门槛：被至少 N 篇种子共引才拉元数据（默认 3）
    --min-size N   簇至少多少篇才显示（默认 5）

第一次用一条新窄带：先写 band.json（见 pipelines/direction_map/CLAUDE.md），
...
def main():
    ...
    if action == 'seeds':
        ...
        print('')
        print('下一步: python 找新文献/方向地图.py build --band %s' % band)
        return 0 if r['total'] else 1

    if action == 'wechat':
        d = opt('--dir') or pos(1)
        if not d:
            print('用法: python 找新文献/方向地图.py wechat --band X --dir <公众号md目录>')
            return 2
    ...
    if action == 'build':
        ...
        print('下一步: python 找新文献/方向地图.py cluster --band %s' % band)
        return 0

    if action == 'cluster':
        dm.cluster(band, min_size=int(opt('--min-size', 5)), progress=print)
        print('下一步: python 找新文献/方向地图.py report --band %s' % band)
        return 0
