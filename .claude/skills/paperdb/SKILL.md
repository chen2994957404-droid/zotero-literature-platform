---
name: paperdb
description: 抽取出来的结构化记录 → 可筛可查、能比大小的数值库。什么时候用：用户要比数字：「拉伸强度超过 10 MPa 的有哪些」「自修复效率最高的几篇」；用户要看覆盖情况：「哪些篇有合成条件但没有性能数据」「精层抽了多少篇」；你要做横向关联分析、找「机理 × 性能」的空白格
---

<!-- 本文件由 host/codegen/skills.py 生成，**别手改**。改源：tools/paperdb/SKILL.md + tools/paperdb/tool.toml -->

> **动手之前先看这三行**（取自 `tools/paperdb/tool.toml`）：
> 不花钱 · 无副作用（只读） · 任何机器都能跑
> MCP 暴露方式：`tool`（模型可以自己调）
> 命令行：`python -m tools.paperdb`

# paperdb —— 查抽取出来的结构化数值

## 什么时候用我

- 用户要**比数字**：「拉伸强度超过 10 MPa 的有哪些」「自修复效率最高的几篇」
- 用户要**看覆盖情况**：「哪些篇有合成条件但没有性能数据」「精层抽了多少篇」
- 你要做横向关联分析、找「机理 × 性能」的空白格

只读、免费、秒回，你可以自己调。

## 怎么用

| 工具 | 用途 |
|---|---|
| `paperdb_find` | 按关键词 / 档次 / 某字段有值 / 性能数值范围筛 |
| `paperdb_stats` | 有多少篇、各字段有值率（**先看这个再下结论**）|
| `paperdb_props` | 抽到过哪些性能名、各多少条、范围多大 |
| `paperdb_samples` | 样品层：一行一个配方（这篇里有几个体系、各是什么组成）|
| `paperdb_measurements` | 测量层：一行一个数字，**带测试条件与出处**；`located=true` 只要能追溯到原文的 |
| `paperdb_provenance` | 这库里的数字有多少带出处 / 带条件 / 挂到了具体样品 |
| `paperdb_curves` | 曲线层：从论文图里抠下来的曲线（哪篇第几张图、多少个点、多确信）|
| `paperdb_curve_points` | 某条曲线的原始点，要画图或再分析时用 |
| `paperdb_sql` | 复杂查询自己写 SQL（只接受 SELECT / WITH）|

用 `paperdb_props` 先看性能名怎么写的，再拿去 `paperdb_find`：
性能名是从原文里抽的（常见的那些会归到统一词表），猜名字容易筛出空。

**要把数字写进论文/报告，先 `paperdb_measurements(located=true)`** ——
没有出处的数字只能当线索，得自己翻回原文核。`paperdb_provenance` 告诉你这库整体有多少能追溯。

## 什么时候**别**用我

- **筛完要写进库** → 不行，我是只读的；数据要改就去改 `structured/<key>.json` 再 rebuild
- **想读整篇内容 / 摘要 / 机理讨论** → 我只有结构化字段；正文去 `library_fulltext`，
  中文精读报告在 `library/<key>/summary.html`
- **库里没有这篇** → 先 `extract` 抽（花钱），或先让用户精读它
- **要跨单位比较**（MPa vs kPa）→ 我**故意不做单位换算**。把单位一起筛，
  或者自己换算并说清楚，别默认数字可以直接比大小
- **一句话的综合回答** → `ask`

## 边界

- 数值拆自「人话字符串」，拆不出数字的记录 `value` 是 NULL —— `min_value` 筛不到它们
- **从曲线抠来的数字**（`method='curve'`）是**读图读出来的近似值**，`cmp='~'`：
  能用来排序和找规律，写进论文前要回原图核
- **v1 老记录没有样品也没有出处**（`method='text-v1'`、`sample_id='main'`、`location` 空）：
  它们的数字只能算「这篇里出现过这个值」，不能算「这个配方的性能」
- `tier`（精层/粗层）差别很大：粗层来自 Zotero 全文索引 + 本地小模型，
  **只适合粗筛**，下结论前先看 `paperdb_stats` 里那一档的有值率
