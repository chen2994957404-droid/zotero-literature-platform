---
name: extract
description: 一批文献 → 统一 schema 的结构化字段 + 横向对比表。什么时候用：用户说「把这批文献的数据抽出来」「做一张对比表」；你要给 paperdb 补料：想筛的字段在库里还是空的
---

<!-- 本文件由 host/codegen/skills.py 生成，**别手改**。改源：tools/extract/SKILL.md + tools/extract/tool.toml -->

> **动手之前先看这三行**（取自 `tools/extract/tool.toml`）：
> **会花钱** · **有副作用**：写 data/serving/structured/<key>.json 与 compare*.md · **只能在运行端（主力机）跑**
> MCP 暴露方式：`prompt`（**由人在客户端点，模型不能自己发起**）
> 命令行：`python -m tools.extract`
...
`python -m tools.extract.audit`（免费、秒级）：把抽出来的每个数字回原文逐字找，
按可疑程度排序。**给用户报数字之前先看一眼这个** ——
命中率低的那几篇，它们的数值只能当线索。
