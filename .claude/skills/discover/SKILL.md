...
- `--精排`（2026-09-19 加，默认关）：前 60 篇新文献再让 Jev 判断模型打「贴题 0–3 档」重排。
  向量只认字面像不像，「主题一样但说法换了」的会被它捞回来（对照实验前 50 名命中 76% vs 64%）。
  要 `TYPESAFE_KEY`（主力机有），190 篇约 0.006 美元；没配就自动跳过、照常出结果

## 找新方向时（2026-09-13）

加 `--新方向`。默认排序 = √(跟他的库像不像 × 跟本次主题贴不贴)，离库远的会被压下去 ——
深耕现有方向时对，找新方向时反。`--新方向` 只按贴题排，「离你的库」一栏变成摆给他看的事实：
**远 = 他库里还没有这一类，正是找新方向要的。**
