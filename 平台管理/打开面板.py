...
1. **2 秒是猜的**。机器慢一点、第一次导入慢一点，浏览器就赶在服务器绑好端口
   ...
   永远只是在赌**。
2. **面板真的起不来时，用户什么也看不到**。`panel_launch.py` 会把 traceback
   写进 `平台管理/panel_launch.log`，但没人知道该去看那个文件。

所以改成「**轮询到真的能连上为止**」，并在失败时直接把日志尾巴打出来。

用法：双击「控制面板.bat」，或 `python 平台管理/打开面板.py`
"""
import os
import subprocess
import sys
...
def start_panel():
    ...
    subprocess.Popen([exe, os.path.join(HERE, 'panel_launch.py')],
...
def main():
    ...
    print()
    print('启动日志的最后几行（平台管理/panel_launch.log）：')
    print('─' * 60)
    print(log_tail())
    print('─' * 60)
    print()
