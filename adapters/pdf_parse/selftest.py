...
用法: python adapters/pdf_parse/selftest.py
"""
import sys, os, tempfile
from adapters.pdf_parse import parse_pdf, is_parsed, PDFParseError
...
def main():
    ...
    #    注：安全化后 _token() 会回退到 core.config 读 .env，这是期望行为，
    #    所以测"能拿到 token"；真的没有时才验证报错（且错误信息要有修复指引）。
    from adapters.pdf_parse import _token
    try:
        t = _token()
        if t:
