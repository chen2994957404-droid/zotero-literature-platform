# -*- coding: utf-8 -*-
"""deepread · 精读编排（定理）：一个 Zotero item key → 一份中文图文精读。

**这块是阶段 3 的主角。**在它之前，这条工作流是 `zotero_watcher.py` 里
五个子进程调用串起来的，接口是「脚本路径 + 参数顺序」：拉起 mineru_parse.py
（给它 pdf 路径和输出目录）、拉起 deepread_v4.py（给它解析目录、输出文件、
provider、model、key —— 全靠位置区分）、再拉起 si_deepread.py、merge_summary.py。

于是：改个参数次序运行时才炸；失败只拿得到退出码和一坨 stdout；
中断了不能续跑；面板看不到进度；不知道某份产物是谁产的。
...
**它不写 Zotero、不改标签**（那是界面环的事，也是两台机器分工的闸门所在）。
`r.state` 是「实际做成了什么」的事实，标签怎么打由调用方决定。
"""
import json
import os
import time
...
from pipelines.deepread.si import SIFailed                # noqa: F401
