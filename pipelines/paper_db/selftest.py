# -*- coding: utf-8 -*-
"""paper_db 自测：不碰真实数据、不调任何服务，验建库 / 筛选 / 只读约束。"""
...
def main():
    ...
    with tempfile.TemporaryDirectory() as d:
        real_struct, real_db = paths.STRUCTURED, paper_db.db_path
        paths.STRUCTURED = os.path.join(d, 'structured')
        os.makedirs(paths.STRUCTURED)
        paper_db.db_path = lambda: os.path.join(d, 'papers.db')
        paper_db.close()
        try:
            for r in RECS:
            ...
            total += 1
            n_paper, n_prop = paper_db.rebuild(log=lambda *a: None)
            ...
            total += 1
            hit = paper_db.find(prop='tensile', min_value=10)
            ...
            total += 1
            hit = paper_db.find(field='synthesis_conditions')
            if [r['key'] for r in hit] == ['AAAA0001']:
            ...
            total += 1
            rows = {r['key']: r for r in paper_db.query('SELECT key, tier, is_review FROM papers')}
            ...
            total += 1
            try:
                paper_db.query('DELETE FROM papers')
                print('  [FAIL] 居然让写语句跑了')
            except ValueError:
            ...
            again = paper_db.rebuild(log=lambda *a: None)
            if again == (3, 3):
            ...
            paper_db.close()
            paths.STRUCTURED, paper_db.db_path = real_struct, real_db
