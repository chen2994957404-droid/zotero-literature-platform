# -*- coding: utf-8 -*-
"""direction_map 自测：不联网，用假数据把 落库 → 聚类 → 报告 走一遍。
用法: python pipelines/direction_map/selftest.py
...
try:
    print('== 1. 空库时的行为 ==')
    check('没建库时 stats 说 exists=False', dm.stats().get('exists') is False)
    try:
        dm.cluster()
        check('空库聚类应报错', False)
    ...
    print('== 3. 聚类 ==')
    rep = dm.cluster(resolutions=(1.0, 2.0), min_size=2, tries=3, progress=None)
    check('报告两档', len(rep) == 2, str([r[0] for r in rep]))
    s = dm.stats()
    check('聚类结果已落库', s['clustered'] == 15, str(s['clustered']))

    print('== 4. 报告 ==')
    txt = dm.report(min_size=2, top_refs=3)
