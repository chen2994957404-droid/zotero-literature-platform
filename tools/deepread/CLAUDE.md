...
> 跟他讲话不要提技术细节，用通俗表述说结果。技术决策你自己拿主意。
...
1. **`run()` 不写 Zotero、不改标签。**它只产出文件；`r.state` 是「实际做成了什么」
   这个**事实**，翻译成标签是 `tags.py` + `host/watcher/` 的事
   （也是两台机器分工的闸门所在）。
...
「打个标签就全自动」那条轮询服务住在 **`host/watcher/`**，不在本工具里。
因为它精读完还要顺手调 `tools.extract` —— 跨工具编排属于 host（硬规则 2 与 4），
判工作流由写在 `host/watcher/__init__.py`。本工具只管「精读一篇」这件事。
