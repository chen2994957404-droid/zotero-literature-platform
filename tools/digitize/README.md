...
**原子能力**：图表图片 → 数据点（X-Y 数值）。把论文图里的曲线/散点抠成可用数值，
解决"定量数据收集"痛点。

## 接口
```python
from tools.digitize import digitize
...
## 抠出来的数值会留住（2026-09-06）

`digitize_paper()` 读完的图存进 `curated/<key>/curves.json`：

- **抠过的图不再重抠**（`refresh=True` 才强制重读）—— 图只需花钱读一次
- 一次只读几张（`only=[2,3]`）时是**并入**，不会抹掉上次读过的其它图
- 读失败的那张不存 —— 存了会让「这张抠过了」变成假的
- `tools/paperdb` 会把它编进查询库：曲线本身进 `curves` 表，
  峰值派生成测量（`method='curve'`，出处 `Fig. 3`），跟文字里抽的数字放在一起比大小

真相是那个 JSON 文件，库随时可重建。
...
- 底层可换、上层不动：只改 provider/model 参数。见 docs/reference/视觉模型选择_参考.md。
...
## 自测
```
python tools/digitize/selftest.py
```
