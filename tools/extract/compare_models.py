...
用法:
  python -m tools.extract.compare_models                    # 自动挑 3 篇，本地 vs 云端
  python -m tools.extract.compare_models --n 5              # 挑 5 篇
  python -m tools.extract.compare_models <KEY> <KEY> …      # 指定篇目
  python -m tools.extract.compare_models <KEY> --only-local # 只跑本地（不花钱）

为什么要有这个（2026-08-28）：「本地 qwen 够不够用」这种问题，
**光靠印象回答是不可信的**（架构准则调研先行原则）。跑几篇、把两边的字段并排摆出来，
