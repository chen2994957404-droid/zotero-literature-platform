# -*- coding: utf-8 -*-
"""按 DOI 把文献收进 Zotero（走 Web API 写入；本地 API 只读，见踩坑 #8）。

流程：DOI → Crossref 取元数据 → 组装 Zotero item → 走适配层 POST。

**这是有副作用的操作**（会改用户的 Zotero 库并同步到所有设备），
所以默认不打任何标签 —— 是否触发精读由用户单独决定（精读要花钱）。

用法:
  python -m tools.discover.importer 10.1021/acsami.2c04994
  python -m tools.discover.importer <DOI1> <DOI2> ... --tag 待处理
...
from shared.kernel.cli import flag, opts, positionals
...
def import_dois(dois, tags=None, verbose=True, forced=False):
    ...
    for doi in dois:
        ...
        if not doi:
            continue
        try:
            it = crossref.to_zotero_item(crossref.work(doi), tags)
            if not it['title']:
            ...
            failed.append((doi, 'Crossref 查不到这个 DOI'))
