...
def c_no_popup():
    ...
    没加静默标志就会不停闪蓝色窗口打扰用户。
    正确做法：走 shared.kernel.subproc，或显式带 creationflags。
    **这一项是防复发的关键** —— 光修好现有的 17 处不够，得让以后写错立刻被发现。
    ...
    if bad:
        return WARN, f'{len(bad)} 处子进程调用可能弹窗（改用 shared.kernel.subproc）: {bad[:6]}'
...
def c_secret_storage():
    ...
    """
    from shared.kernel.config import keyring_status, key_location, SECRET_KEYS
...
def c_hardcoded():
    ...
    for f in code_files():
        ...
        # config 自己要写默认值；本文件里就写着要查的模式串本身（自匹配）
        if (os.path.join('kernel', 'config') in n
    ...
    return (OK, '没有硬编码的地址，配置都走 shared/kernel/config') if not bad         else (FAIL, f'{len(bad)} 处硬编码: {bad[:4]}')
...
# ══════════════════════════════════════════════════════════════════════
# 离线档：不依赖任何外部服务，永远应该全绿（见 docs/explain/架构重构_v2总体设计.md 第五节）
# ══════════════════════════════════════════════════════════════════════
def c_role():
    """本机角色配没配、配的是什么。见 docs/howto/两台机器的分工.md。
...
def c_installed():
    ...
    from shared.kernel import paths as _cp
