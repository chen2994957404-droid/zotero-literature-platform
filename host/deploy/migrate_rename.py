...
**它就住在项目里，直接跑**。2026-09-08 之前它被复制到 %TEMP% 再启动，
理由是「不能重命名自己住的目录」—— **那个理由是我没实测就想当然的，实测是错的**：
真正挡住重命名的只有「有进程把**工作目录**设在里面」，而 Python 读完脚本就把文件句柄
关了，cmd 执行 .bat 也不锁父目录（两条都在 2026-09-08 实测过）。
那套多余的复制反而制造了一个真事故：copy 失败没人发现，于是跑的是上次留下的旧副本，
新写的修复根本没生效（踩坑 #147）。**所以现在直接跑，配套的 .bat 只负责
`cd` 到项目的上一级再起 Python。**

**为什么它不守那几条强制规范**（只依赖标准库、不走 shared.kernel.cli/log）：
...
**给用户看的字一律在这里，不在 .bat 里**：cmd 处理 UTF-8 中文不可靠，
会把整个批处理的行切错位（踩坑 #147 现场：`echo` 的中文变乱码后被当成命令执行）。
所以那个 .bat 是纯 ASCII 的。
...
# 版本戳：开头就打印出来。**这是为了让「跑的到底是哪一版」一眼可见** ——
# 2026-09-08 就是因为无从判断，才让一份旧副本悄悄跑了一整轮（踩坑 #147）。
VERSION = '2026-09-08c'
...
def main():
    ...
    print('=' * 62)
    print("""
这一步只动你现在这台机器。B 机（主力机）完全不受影响。

开始之前只要做一件事：**把 Claude Code 关掉**（编辑器、停在这个
文件夹里的命令行窗口也一并关掉）。

控制面板的后台进程不用管，我会自己停掉它 —— 那个进程没有窗口，
你在任务栏上是看不见的。

万一还有别的程序占着，我会把它的名字告诉你然后停下，不改坏任何东西。
""")
    try:
        if input('  准备好了就按回车开始，直接关掉窗口就是取消：') is None:
            return 1
