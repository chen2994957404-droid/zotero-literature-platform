...
# 两个地址，**宽带是主路**（2026-09-04 用户决定 B 以后不连 WiFi 了）：
#   211.83.153.16    宽带的有线 —— 稳。用户原话「宽带比较稳定」
#   192.168.123.216  家里路由器的 WiFi —— 更快但不稳，留着当第二条腿
#
# 顺序不是形式：排在前面的先试，连不上要等满 ConnectTimeout 才换下一个。
# 把常年断着的那个放前面，等于每条命令都白等十秒。
#
# ⚠ **整晚「A 连不上 B」的谜团就是这个**（2026-09-04 用户一句话点破）：
#   A 只在家里的 WiFi 网段上，所以**只能通过 192.168.123.x 找到 B**。
#   B 的无线一断，A 就彻底看不见它 —— 而 B 本人**一直活得好好的**，
#   靠有线继续上网干活。这解释了此前所有对不上的观测：
#     · B 从没睡过（零条电源事件）✓
#     · 断线那二十分钟里 B 正常跑完了一篇精读、正常调用 Google API ✓
#       （它走有线出去，外网通得很，只是家里那条腿断了）
#     · 我先后误判成睡眠、网卡省电、A 侧代理、无线驱动 —— **全错** ✓
#
#   教训：**「连不上」的第一问不是「它怎么了」，而是「我走的是哪条路，
#   它还在不在那条路上」。** 一台多网卡的机器，不同的路通往不同的它。
#
# 顺序：局域网在前（同网段更快），宽带地址兜底。
# 曾试过 Tailscale 做兜底，装上能用、故障演练也过，**但把用户的 VPN 挤断了**
# （虚拟网卡抢默认路由与 DNS），已卸载 —— 能用 ≠ 能共存。
# 现在这个方案不装任何软件，用的是 B 本来就有的第二条腿。
#
# ⚠⚠ 211.83.153.16 是**公网地址**，意味着 B 的 22 端口暴露在那个网上，
#   而 `sshd_config` 里 `PasswordAuthentication` 那行是**注释掉的** ——
#   注释掉等于用 OpenSSH 的默认值，**默认是 yes**。也就是说
#   **现在任何人都能对着这个公网地址试密码**。防火墙规则也是 Any 档。
#   （我差点在这里写「已确认关闭」然后就过去了 —— 没验过的话不许写进注释，
#    这正是今天踩了五次的那个毛病。）
#   待办里记着这条，要么关掉密码登录，要么把防火墙收窄到具体网段。
#   另外换网络时这个公网地址会变，到时候这一行要跟着改。
_DEFAULT_HOSTS = '211.83.153.16,192.168.123.216'
...
def diagnose(stderr):
    ...
    if 'connection timed out' in e or 'no route to host' in e:
        ...
        return ('B 机没应答。**别先认定它睡了**，也**别拿 ping 当判据**：\n'
                '\n'
                '  ⚠ B 的防火墙不回 ICMP —— 2026-09-04 实测：ping 收到 0 个回包的'
                '同一秒，\n'
                '    SSH 连得好好的。所以「ping 不通」在这台机器上**什么都不能证明**，\n'
                '    我曾拿它当过证据，白查了一轮。要探活只能用 22 端口本身。\n'
                '\n'
                '  几种真实原因，症状一模一样：\n'
                '  ① 本机代理/VPN 接管了到局域网的路由（两台都靠代理上网）。\n'
                '     判据：`Find-NetRoute -RemoteIPAddress ' + HOST + '` 看走的是哪块网卡；\n'
                '     走的不是局域网那块就是它。解法：把网段加进代理的直连规则。\n'
                '  ② B 换了 IP（笔记本换网络时会）。\n'
                '  ③ 无线本身抽风 —— **但别再拿驱动事件当证据**：那 322 条\n'
                '     Netwtw14 事件全是「信息」级，零错误零警告（踩坑 #109）。\n'
                '  ④ 真的关机或睡眠 —— **这条要拿证据**，别猜：\n'
                '     `Get-WinEvent -ProviderName Microsoft-Windows-Kernel-Power`，\n'
                '     没有电源事件就说明它压根没睡过。\n'
                '\n'
                '  这一条跟密钥、账号都无关，别往那个方向查。\n'
                '  **另外：连不上是常态而非异常，作业一律用 `job ... --async` 发** ——\n'
                '  交给 B 的计划任务跑，断线不影响它干活，重连后取结果。')
