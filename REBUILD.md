...
**组织原则从「按技术分层」改成「按工具包」**:每个工具是一个自包含的包,
...
├── prompts/            <名>_v<N>.txt（main_v2.txt / si_v1.txt）**只增不改**
├── evals/
│   ├── golden/         金标输入 + 期望
│   ├── scorers/        评分器
│   └── thresholds.toml
...
| 提示词 | `tools/<t>/prompts/<名>_v<N>.txt` |
| 共用件的提示词 | `shared/<环>/<b>/prompts/<名>_v<N>.txt`（R5 窗新增：`query_expand` 被两个工具用，它的提示词没地方去）|
...
免费只读的东西必须是 `tool`,所以它需要一个 `costs_money=false` 的工具包来装。
...
### launch/ ← 根目录 7 个 .bat（R4 窗又添了一个）
`控制面板.bat` `更新平台.bat` `诊断报告.bat` `精读监听.bat` `比一比两个模型.bat` `重抽缺SI的文献.bat` `重跑精读PRO.bat`
...
### docs/
- `explain/` ← 架构准则_第一性原理 · 对标报告 · 正文数据关联_方向说明 · 架构重构_v2总体设计 · 模块采购清单
- `howto/` ← 代码规范_标准脚本模板 · 两台机器的分工 · 测试端与阶段3路线
- `reference/` ← 数据契约 · API_verified · 架构总览 · 演进规划与模块地图 · 视觉模型选择_参考
- `incidents/` ← 踩坑记录(R7 窗按工具包,总账留这里)
- 原地不动:`变更记录.md` `待办与需求.md`

### data/ ← `workflow_data/`(R6 窗,只在 A 机)

> **R6 窗实际落位与本段有一处不同**：切口划在**目录边界**而不是文件上 ——
> 整个 `parsed/` 与 `si_parsed/` 进 `raw/`（`full.md` 也在里面），
> 只有我们自己产出的 `meta.json` / `summary*.html` 进 `curated/`。
> 理由：`full.md` 是 MineRU 直接吐的，且与 `images/`、`layout.json`+`*_origin.pdf`
> 物理绑死，拆开会逼着改工具逻辑，违反本节「只动目录组织」。**规则优先于映射表。**

- `raw/` ← MineRU 原始解析产物（整个 `parsed/` 与 `si_parsed/`）
...
| R3 | 切出 ask / askworld / discover / snowball / direction / curate | [x] | **根目录再无中文代码目录，也无 `pipelines/`。** 五个工具成包：`ask`（含两条向量化线合并）· `askworld`（问答 + 纯检索）· `discover`（paper_discovery + lib_match + discover/collect/import_by_doi/thesis）· `direction`（direction_map + 方向地图 + brainstorm）· `curate`（7 个脚本 → sync/junk/rename/backfill/tags 五模块）。**snowball 判定为纯 API 包装**（`_get`/`_norm` 全是转发 openalex，`expand()` 只有循环去重限速，无算法无跨块编排）→ 整块留 `shared/adapters/snowball/`，唯一那点编排留在 `discover.snowball_more()`，**不单独成工具，故共 9 个工具**。**两处偏离第四节映射表**（踩坑 #82）：`query_expand` → `shared/adapters/`（两个工具用，且 import llm_client 进不了 domain）、`lib_match` → `tools/discover/match.py`（只有一个使用者）——照映射表会逼出 `tools`→`tools` import，违反第三节硬规则 2，**规则优先于映射表**。顺手补掉四处「联网不在 adapters」：新建 `shared/adapters/crossref/`（必备文件齐）、新增 `llm_client.chat_messages()` / `zotero_client.alive()+library_index()` / `embed.alive()`。删 `find_papers.py`（`search()` 的薄壳）。配套改 pyproject / paths / panel（删两处 sys.path 借用）/ health_check（KEY_MODULES 13 个，`find_script` 退休）/ handover 生成器 / README / CLAUDE.md。**两条联网守卫从 R1 起一直在空转**（踩坑 #83），当场改指新环路径并故意违反一次验证会红。**pytest 194 过 1 跳（同基线）· 离线体检 10/0/0 · 完整体检 16/2/0 · 真跑一次 discover 返回 14 篇。** 依赖方向守卫、docs 旧路径、sync 驱动两个工具 → 记进待办等 R7。 |
| R4 | 每工具补齐必备文件(tool.toml / README / SKILL / cli / mcp) | [x] | **十个工具形状一致了，MCP 从「手写清单」变成「读 tool.toml 聚合」。** 必备文件 ×10 全补齐；命令行统一成 `python -m tools.<名>`（`paperdb/query.py`、`discover/find.py` → `cli.py`；`ask/askworld/direction` 的 `__main__.py` → `cli.py`；`deepread/extract` 的 `main()` 从 `batch.py` 搬出；`curate` 新写子命令分发；`digitize` 新写命令行 + `digitize_file()`），`__main__.py` 一律退化成一行壳。**新建 `tools/library/`（第十个工具）**：`host/mcp/zotero_server.py` 解散，9 个只读查询成为它，`ping` 留给服务自己，拼 API 路径/读 Total-Results 头/压平条目**下沉进 `zotero_client`**（原先在 host 里裸 urlopen，是「联网只在 adapters」的破口）。**偏离第一节的 9 个工具清单**：第四节本就留了「新建 `tools/library/` 或并入 `ask`」两个选项 —— 并入 `ask` 会让这些免费只读查询跟着 ask 降级成 prompt（ask 花钱），**agent 从此不能自己检索文献库，是真实功能倒退**，所以单独成工具（同 R3 的「判据优先于映射表」）。`host/mcp/` 新增 `server.py`（聚合入口）+ `registry.py`（读 tool.toml、挂 mcp.py、校验自洽）；`stdio.py` 补齐 resources/prompts 两族方法，**报文形状核对过官方 schema 2024-11-05**。**安全铁律落地**：`costs_money` 或 `side_effects` 非空的工具包**不许注册 tool 类**（tool 是模型可自己调的），`registry.check()` 强制 —— 现状 tool 只有 library×9 与 paperdb×4，resource 是 extract 的 3 张对比表，其余 8 个工具包各 1 条 prompt。配套：`kernel/cli` 加 `wants_help()`（起因见踩坑 #85：`--help` 曾直接触发全库抽取）、新增 `kernel/mcp_prompt.py`（八条 prompt 共用同一段「先问用户」）、体检关键入口 15 个、`sync.py` 改调 `tools.extract`、`重跑精读PRO.bat` 改新入口。**pytest 194 过 1 跳（同基线）· MCP 自测 29/0 · `server.py --list` 三类齐全且自洽 · 离线体检 10/0/0 · 完整体检 16/2/0 与 R1~R3 基线一致。**踩坑 #84 #85 #86。工具的 `tests/`、`evals/`、`tool.toml` 的 `prompts` 字段、B 机 MCP 重新注册 → 记进待办等 R5/R7。 |
| R5 | prompts 与 evals 归位到各工具 | [x] | **提示词从「散在 .py 里的字符串」变成「带版本、只增不改的文件」，评测集回到它服务的那个工具里。**新增原子模块 `shared/kernel/prompts/`（必备文件齐）：`load(owner, spec)`，owner 带斜杠 = 相对仓库根的包路径，不带 = 工具名；在 kernel 不在 adapters，因为它只读一个文本文件、外部世界变了它不用改。**搬了 11 段提示词，用 import 取值再写盘、一字不差**（逐条核对字数；精读正文那段与 git HEAD 原文件逐字节相同）：deepread `main@v2`(git mv)/`si@v1`、extract `main@v1`/`eval@v1`、ask/askworld/digitize `main@v1`、direction `brainstorm@v1`、curate `autotag@v1`、query_expand `english`/`survey`/`problem@v1`。deepread 两处 `PROMPT_VER` 现在直接参与拼版本（`f'main@v{PROMPT_VER}'`），版本号与实际用的那一版不可能再各走各的。各 `tool.toml` 的 `prompts` 填上真值，`registry.check()` 加一条「声明的版本必须真在盘上」（故意改成 `main@v9` 验过会红）。**两处判断**：① `schema` 的 `SYS`/`EVAL_SYS` 归 extract —— schema 回答「抽什么字段」，不回答「怎么跟模型说话」，且只有 extract 一个使用者；`build_user_prompt` 留在 schema（它要遍历 SCHEMA）。② `query_expand` 是共用件（askworld+discover 都用），它的三段长提示词照样迁出，**第三节位置表因此多了「共用件的提示词」一行**（同 R3/R4：规则优先于表的覆盖范围）。**`adapters/evalset` 解散 → `tools/deepread/evals/`**：它只服务一个工具（`shared/` 门槛是 ≥2 个），且根本不是 adapter —— 不联网、不包装外部服务，它是「怎么评价这个工具的产出」。拆成 `scorers/quality.py`（`metrics(html)` 纯函数，不要真实文件就能测）+ 评价存储 + `thresholds.toml`（阈值 3/3 与废品线 3000 的唯一出处；`stats()` 多返回 `need_good`/`min_good`，面板与交接生成器不再各自硬编码 3）。原 `selftest.py` 6 项改写成 pytest 进 `tools/deepread/tests/`，另加 3 条（内嵌图不许污染数值密度 / 废品线只有一个出处 / 怪 key 只返回 None 不炸）。**其余九个工具建 evals 空骨架**，每份 README 写明「这个工具『好』的判据是什么」（空骨架配空话等于没建）。**pytest 203 过 1 跳（原 194，新增 9 条）· 离线体检 10/0/0 · 完整体检 16/2/0 与 R1~R4 基线一致 · `server.py --list` 自洽 · 面板评价卡片与交接生成器实跑通过。**踩坑 #87 #88。九个空 evals、`%` 数值正则的老毛病、「块」两处枚举口径 → 记进待办等 R7。 |
| R6 | data/ 五层重排 + launch/ + 清死文件 | [x] | **`workflow_data/` 平铺目录 → 五层 `data/`，全系统只有 `paths.py` 一个文件跟着改了布局** —— 这是 v2 立 `paths.py` 的目的第一次真正兑现。`library/<KEY>/{parsed,si_parsed}` → `data/raw/<KEY>/`；`meta.json` 与 `summary*.html` → `data/curated/<KEY>/`；`{structured,vector_db,direction}` → `serving/`；`{state.db,papers.db,evalset.json,_last_search.json}` → `state/`；`logs`/`backup` 平移。分层判据是**「重建它要付出什么」**，写在 `paths.py` 顶部。**偏离第四节映射表一处**（同 R3/R4/R5「规则优先于表」）：表把 `full.md` 归 curated，实际归 **raw** —— 它是 MineRU 直接吐的、没经我们加工，且与 `images/`（Markdown 相对链接）、`layout.json`+`*_origin.pdf`（`crop_figures()` 要求三者同目录）物理绑死，单独拎出来会**逼着改工具逻辑**，而本窗明令只动目录组织；所以切口划在**目录边界**上。**新增 `host/deploy/migrate_data.py`**（B 机才是权威副本，同一次搬家必须两台结果一致）：预演 → `--apply`（复制 + 逐项核对文件数与字节）→ `--remove-old`（核对全过才删）；A 机跑完前两档，15 项 / 109 文件 / 127.2 MB 全部一致。`paths.LIBRARY` 退休拆成 `CURATED`/`RAW`，8 个调用点各自表态；新增 `paper_raw_dir()`；`artifact_gaps` 算体积改走两个目录。**守卫跟着改名否则静默空转**（同踩坑 #83）：路径守卫的字面量从 `'workflow_data'` 改成 `['"](?:workflow_)?data['"/]`，体检扫密钥的 `glob('*.bat')` → `glob('launch/*.bat')`，**故意违反一次确认会红**。7 个 .bat → `launch/`（文件名保持中文），`%~dp0` → `%~dp0..`，交接生成器加 launch/ 分支（原先按 .py 数永远显示「0 个脚本」）。`.gitignore` 重写成「整个 `data/` 忽略 + `!` 放回三样重建不了的」，6 个跟踪文件被 git 识别成 rename。清了 43 个 `__pycache__`/`.pytest_cache`/`egg-info`/`panel_launch.log` 并重装。**pytest 204 过 1 跳（同基线）· 离线体检 10/0/0 · 完整体检 16/2/0 与 R1~R5 一致（原子模块自测 24/29 → 26/29，涨的两个正是被踩坑 #89 打断、本窗修好的 ask 与 extract）· 面板 HTTP 200 · `cmd /c launch\诊断报告.bat` 实跑通过 · `server.py --list` 自洽。**踩坑 #89 #90。**遗留**：A 机 `workflow_data/` 原目录还在（删除被拦，需用户跑一次 `--remove-old`）；B 机自启任务路径要重注册；docs 散文里的旧路径与旧 .bat 名 → R7。 |
| R7 | 守卫重写 + .claude 全生成 + docs 归类 + 终验 | [x] | **规则从此有执行者，重构完成。守卫 15 条 → 23 条。****① 先把地扫干净**：四处硬规则违规清零 —— `watcher`/`watchdog` → `host/watcher/`（判据是第一节对 host 的定义本身：不是能力，是让平台活着的东西；硬规则 4 明写 host 可 import 一切）；`extract → paperdb.rebuild()` 改成**索引自己保新鲜**（`paperdb._ensure_fresh`：query 时比一眼时间戳，比源 JSON 旧就重建）—— 真正的毛病不在那行 import，而在**责任放错了地方**，让写入方负责就得每个写 JSON 的人都记得刷一次，漏一个用户就查到旧数据**且不报错**；`wizard` 改调 `schema.coverage`（与 `paperdb.stats` 同一个函数）；`tools/curate/sync.py` → `host/autosync/`（它走子进程躲过了守卫 —— **形式上的规避不是本质上的合规**，踩坑 #93）。下沉规则同样清零：`shared/domain/{si_filter,bibliometrics}` 各只有一个使用者，搬进 `tools/{deepread,direction}/`；adapters 里的单使用者块**不动**，硬规则 3「联网只在 adapters」优先。**② 三条守卫一直在空转**（自己踩到踩坑 #83 的同一个坑）：依赖方向守卫的 `RINGS` 与引导脚本守卫的 `_PROJECT_PKGS` 还写着 `core/domain/adapters/pipelines/apps`，R1 起空转六个窗；「domain 不许有 I/O」一直在 `pytest.skip`（找根目录 `domain/`）—— **skip 和空转是一回事**，报告里那个孤零零的 `s` 谁都看见谁都以为是暂时的（踩坑 #92）。`_imports_of` 还漏掉 `from shared.kernel import paths` 这种最常用写法。新增五条（工具不许互相 import · 必备文件齐全 · `tests/` 不许空壳 · 花钱的不许 `expose=tool` · 下沉规则，含**豁免名单自己也被守着**）+ 两条同步守卫（`.claude/` 与 `docs/incidents/` 必须与源一致）。**六条新守卫逐条故意违反一次验红**，还原用「读进内存→写回原字节」不碰 git（踩坑 #90）。**③ `.claude/` 全生成，手写即违规**：原来四份手写 skill 提着 `core/`、`平台管理/`、`workflow_data/`，这些目录 R1~R6 已经全没了，**而模型每次会话都在读它们**；源搬去 `docs/howto/{skills,rules}/` 与各 `tools/<t>/SKILL.md`，`host/codegen/skills.py` 生成 —— 工具那族不是复制，frontmatter 由 `tool.toml` 生成、正文前自动加「花不花钱/有什么副作用/需要哪档角色/MCP 暴露方式」。守卫上线当场抓到一次真事故（改了四个 tool.toml 忘了重新生成）。**④ docs 四类 + 坑按工具包**：43 个文件的旧引用一次改完，各 CLAUDE.md 里的 `core.paths`/`平台管理/`/`workflow_data` 另修 26 个；十个工具各建 `INCIDENTS.md`，`host/codegen/incidents.py` 汇总成 `docs/incidents/README.md`，**总账不拆**（现场感是它最值钱的部分）。汇总时发现**总账编号撞车**（#15/#16/#43 各用过两次），按号合并会凭空造出不存在的关联 → 改成按 (坑号,现象) 合并、**不重新编号**（重编会让既有引用静默指错，踩坑 #91）。**⑤ `AGENTS.md` 成为正本**（198 行），`CLAUDE.md` 只剩一行 `@AGENTS.md`，结构树由生成器写入；「块」的判据统一成「有没有 `__init__.py`」，与 `paths.block_dirs()` 一致（踩坑 #87）。`README.md` 按新结构重写。**⑥ 顺手修好**：`tools/digitize/selftest.py` 重写成真正离线的（它此前要本机装着 qwen2.5vl，在编程端永远是红的），体检的 `SLOW_TESTS` 豁免随之清空 —— **自测必须离线、秒级、任何机器上都全绿，做不到的活属于 `evals/`**；看门狗找进程改认模块路径（只匹配 `watcher` 会让它把自己杀掉）；测试跟着它测的东西住，`testpaths` 加上 `shared/` 与 `host/`。**验收：pytest 226 全过 0 跳（R6 是 204 过 1 跳）· 离线体检 10/0/0 · 完整体检 16/2/0 与 R1~R6 基线一致 · 原子模块自测 25/28 · MCP 自测 29/0 且 `--list` 自洽 · 面板 HTTP 200 / 28 块 / 0 告警 · `python -m host.autosync` 实跑。**踩坑 #91 #92 #93。**遗留（见待办「R7 窗留下的」）**：**B 机两个计划任务的命令行必须先改**（`host.watcher.watchdog` / `host.autosync`），否则合并后那两条线会静默停摆；B 机 MCP 重新注册；`figure_crop` 的下沉豁免；`架构重构_v2总体设计.md` 只剩历史价值。 |


## 六、重构完成（2026-08-31）

七个窗全部打勾。`rebuild` 分支上的结构就是最终形态，`main` 未动，B 机不受影响。

**合并前必须做的一件事**（不是可选项）：B 机上两个计划任务的命令行还指着老模块路径 ——

| 任务 | 改成 |
|---|---|
| `ZoteroLiteratureWatcher` | `python -m host.watcher.watchdog` |
| `LiteratureAutoSync` | `python -m host.autosync` |

**不改的后果是静默停摆**：任务照常「成功」运行，只是那个模块不存在了，
精读监听和每小时同步都不再触发，而且**不会有任何报错**。
MCP 注册也要一起更新（R4 起服务入口是 `host/mcp/server.py`，工具名带了前缀）。

后续按工具逐个优化时的入口：**打开一个 `tools/<名>/` 文件夹就够了** ——
代码、命令行、MCP 面、给人的说明、给 agent 的手册、提示词、评测、测试、
这个工具特有的坑，全在里面。这正是这次重构的目的。
---
...
纯 API 包装就留在 adapters,含算法与编排就拆。**在提交信息里写清判工作流由。**
