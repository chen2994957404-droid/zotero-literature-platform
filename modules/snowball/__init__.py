...
FIELDS = 'id,doi,title,publication_year,cited_by_count,primary_location,abstract_inverted_index'
...
def _get(url, timeout=45, retries=2):
    """GET + 429/5xx 退避重试。OpenAlex 偶发限流，批量跑时必然撞上。"""
    last = None
    for attempt in range(retries + 1):
        try:
            ...
            if e.code in (429, 500, 502, 503, 504) and attempt < retries:
                ...
                continue
            raise SnowballError(f'OpenAlex {last}')
        except Exception as e:
            last = f'{type(e).__name__}: {e}'
            if attempt < retries:
                time.sleep(2)
                continue
    raise SnowballError(f'OpenAlex 请求失败: {last}')


def _abstract(inv):
    """OpenAlex 的摘要是倒排索引（词 → 位置列表），要还原成正常文本。

    这是 OpenAlex 的历史设计（规避版权），不还原的话摘要用不了 ——
    而摘要正是 lib_match 判断相关度的主要依据。
    """
    if not inv:
        return ''
    try:
...
def _norm(w):
    """OpenAlex work → 本平台统一的文献字典（与 sciverse.search_papers 同构）。"""
