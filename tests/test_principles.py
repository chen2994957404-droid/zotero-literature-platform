# -*- coding: utf-8 -*-
"""架构准则分级守卫 —— 让「每条规矩都要标可不可推翻」这件事不会随时间烂掉。

2026-09-07 立。起因是用户的疑问：
「那些旧架构准则是不是相当于在你的思考之上，只要稍微合理你也不会推翻它，
我怕它反而会限制大模型的思维。」

这属实：写进架构准则的话会作为用户指令进入每次会话，优先级高于模型自身判断。
解法是给每条规矩标【事实】/【硬约束】/【启发式】，只有前两档不许推翻。

但分级是一次性动作，**没有守卫的话过两个月新加的章节又会变回不带标记的祖训**。
这个文件就是那件事的执行者。它不测功能，只测架构准则这份文件的形状。
"""
import os
import io
import re
...
存档 = os.path.join(ROOT, 'docs', 'explain', '架构准则_v1_历史存档.md')
...
# 调研先行原则的唯一源在架构准则里，被这对标记圈住；下面两份是抄过去的副本。
摘 = ('<!-- 摘:调研先行原则 开始', '<!-- 摘:调研先行原则 结束 -->')
抄 = ('<!-- AUTO:调研先行原则 开始', '<!-- AUTO:调研先行原则 结束 -->')
...
def test_三档标记的定义没被删掉():
    ...
    正文 = _读(架构准则)
    # 只要求「定义表里有」，不要求每档都被用上 ——
    # 【事实】那一档现在确实一次没用：项目事实都住在 AGENTS.md，架构准则里只放判据。
    # 守卫在这里刻意留松，是因为「逼着凑一条事实进来」比空着更糟。
    缺 = [t for t in 标记 if t not in 正文]
    assert not 缺, '架构准则里这几档标记没有定义：' + _NL + _NL.join(缺)
...
def test_启发式那一档写明了可以推翻():
    ...
    assert re.search(r'【启发式】.{0,80}可以推翻', 正文, re.S), (
        '架构准则里找不到「【启发式】…可以推翻」的明文。'
        '没有这句，模型看到标记也只会照办。')


def test_旧架构准则存档没有自称生效():
    """两份文件都自称最高纲领时，模型会照着更长的那份执行。"""
    正文 = _读(存档)
    assert '不再生效' in 正文.split('---')[0], (
        '架构准则_v1_历史存档.md 的抬头没写明「不再生效」，'
        '它会被当成一份 300 行的现行纲领。')
...
def test_调研先行原则只有一个源():
    """三处副本必须与架构准则里的源逐字一致 —— 漂移过一次就再也没人知道哪份对。

    2026-09-07 之前它在架构准则 / AGENTS.md / research-first skill 各写一份。
    现在源在架构准则，其余由 host/codegen/handover.py 抄过去。
    这个测试红了，多半是有人手改了副本 —— 改源，然后跑那个生成器。
    """
    源 = _抠(_读(架构准则), *摘)
    assert 源, '架构准则里找不到 <!-- 摘:调研先行原则 --> 圈起来的源'
    漂 = []
    for f in 副本:
        ...
        本 = _抠(_读(f), *抄)
        if 本 is None:
            漂.append(os.path.relpath(f, ROOT) + '：没有 AUTO:调研先行原则 区块')
        elif 本 != 源:
            漂.append(os.path.relpath(f, ROOT) + '：内容与架构准则里的源不一致')
    assert not 漂, ('调研先行原则的副本漂了。改源（架构准则），再跑 '
