# -*- coding: utf-8 -*-
"""文献查询库的命令行入口 —— **逻辑在 `pipelines/paper_db`**，本文件只解析参数。

把 `structured/*.json` 建成一个能查的 SQLite 库，回答此前答不了的问题：
「含硼、拉伸强度 > 10 MPa 的体系都有哪些」「哪些篇有合成条件但没有性能数值」。

用法:
  python 查询库.py --rebuild                     # 从 structured/*.json 重建（秒级、不花钱）
  python 查询库.py --stats                       # 各档次 × 各字段有值率（数据有多准）
  python 查询库.py --props tensile               # 抽到过哪些性能、各多少条、范围多大
  python 查询库.py --find boron --prop tensile --min 10
  python 查询库.py --field synthesis_conditions  # 这个字段真有值的篇
  python 查询库.py --sql "SELECT tier, COUNT(*) n FROM papers GROUP BY tier"
...
def main():
    ...
    if opt('--props') is not None or flag('--props'):
        _print_rows(paper_db.props(opt('--props')))
        return

    sql = opt('--sql')
    if sql:
        _print_rows(paper_db.query(sql))
        return

    mn, mx = opt('--min'), opt('--max')
    rows = paper_db.find(text=opt('--find'), tier=opt('--tier'), field=opt('--field'),
